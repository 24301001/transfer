...
- 数据库密码、Redis 密码、邮箱授权码、地图密钥和大模型 API Key 必须通过环境变量注入。
- 提交代码前应轮换已经暴露过的密钥，并检查 Git 历史中是否残留敏感信息。
- 生产环境必须限制跨域来源、上传文件类型、文件大小和接口访问频率。
- JWT 密钥应使用足够长度的随机值，并配置合理的有效期与注销机制。
- 上传文件应进行 MIME 类型、扩展名和内容安全校验。
- 异常响应中不得返回密码、Token、密钥、服务器绝对路径或完整堆栈。
- 第三方服务调用应设置超时、重试、熔断或回退逻辑，并保留必要的 Trace ID 便于排查。
- 建议在提交前执行接口回归测试，并同步更新 OpenAPI 与 Apifox 文档。

## 十一、常见问题

### 1. 前端请求返回 401

检查本地是否存在有效 Token、请求头是否包含 `Authorization: Bearer ...`，以及当前账号角色是否拥有页面权限。

### 2. 直接访问 `http://localhost:8080/` 返回 500 或静态资源错误

Spring Boot 后端未配置根路径页面时属于正常现象，应访问具体接口，例如 `/api/v1/health` 或 OpenAPI 文档地址。

### 3. Multipart 请求返回 415 或 500

确认：

- `incident` Part 的类型为 `application/json`；
- 图片使用 `photos`，视频使用 `videos`；
- 整体请求类型为 `multipart/form-data`；
- 不要提交空的文件表单字段。

### 4. 图片或视频无法展示

受保护原始附件需要通过 Axios 读取 Blob；YOLO 标注媒体需要正确配置 `/runs` 代理、静态资源映射及视频 MIME 类型。

### 5. Docker Desktop 无法启动 Redis

检查 BIOS 虚拟化、WSL 2 和虚拟机平台是否启用，并确认 Docker Engine 已正常运行。Redis 连接地址、端口和密码必须与容器配置一致。

### 6. 预测接口长时间无结果

检查预测服务健康状态、接口地址、字段映射、请求超时和服务日志。业务后端只负责预测服务对接，模型推理错误需在算法服务中排查。

## 十二、代码包完整性说明

当前提供的前端压缩包仅包含 `src` 目录，不包含独立运行所需的 `package.json`、`vite.config.*`、`index.html`、锁文件及静态资源目录。正式提交或部署时，应将这些工程文件一并纳入仓库，并确保依赖版本、构建脚本和代理配置可复现。

## 十三、项目状态

当前项目已完成事故上报、附件上传、预测接口对接、AI 问答与即时提示、指挥调度、清障任务、登录注册、个人中心、管理员管理、JWT 校验、Redis 缓存、邮箱与滑块验证、地图定位、天气接入及系统健康检查等主要功能。后续可继续完善自动化测试、容器化部署、接口版本管理、监控告警和生产环境安全配置。

---

如需用于课程验收或项目答辩，建议在仓库中继续补充以下内容：系统页面截图、数据库 ER 图、完整部署拓扑、接口文档地址、测试报告和团队分工说明。
